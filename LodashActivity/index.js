...
const entries = [
  ...
  // Filtrar entradas por el tag Desarrollo, mapear (map) por título y agrupar por autor
function tagsFilter(entry) {
    return lodash.includes(entry.tags, "Desarrollo")
}

const tagsFilteredByDesarrollo = lodash.filter(entries, tagsFilter)
// console.log(tagsFilteredByDesarrollo)

const entriesMapedByTitle = lodash.map(entries, "titulo")
// console.log(entriesMapedByTitle)
